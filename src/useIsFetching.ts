import { getCurrentScope, onScopeDispose, ref, watchEffect } from '@mpxjs/core'
...
export function useIsFetching(
  ...
  if (process.env.NODE_ENV === 'development') {
    if (!getCurrentScope()) {
      console.warn(
        'mpx-query composable like "useQuery()" should only be used inside a "setup()" function or a running effect scope. They might otherwise lead to memory leaks.'
      )
    }
  }
